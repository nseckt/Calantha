...
auto VGATerm::create() -> VGATerm {
  VGATerm term;
  term.buff_   = reinterpret_cast<uint16*>(VGATEXT_ADDR);
  term.col_    = 0x00;
  term.row_    = 0x00;
...
auto VGAColour::as_entry(VGAByte ch) const -> VGAEntry {
  const auto left  = static_cast<uint16>(ch);
  const auto right = static_cast<uint16>(val_);
  return left | (right << 8);
}
...
auto VGATerm::clearscr() -> VGATerm& {
  ...
  for(usize y = 0; y < VGATEXT_HEIGHT; y++) {
    for(usize x = 0; x < VGATEXT_WIDTH; x++) {
      const usize index = y * VGATEXT_WIDTH + x;
      buff_[index] = entry;
    }
  }
...
auto VGATerm::putchar(VGAByte ch) -> VGATerm& {
  ...
  const usize index = row_ * VGATEXT_WIDTH + col_;
